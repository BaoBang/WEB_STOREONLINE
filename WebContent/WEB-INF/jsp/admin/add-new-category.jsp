       ...
       <div class="col-md-12">
        <c:if test="${not empty result and result}">
	        <div class="alert alert-success alert-dismissible">
	        	...
	        	<h4><i class="icon fa fa-check"></i> Thành công!</h4> Thêm thể loại <strong>${category_name}</strong> thành công.
	        </div>
         </c:if>
         <c:if test="${not empty result and result eq false}">
	        <div class="alert alert-warning alert-dismissible">
                <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                <h4><i class="icon fa fa-warning"></i> Thất bại!</h4> Thêm thể loại thất bại.
              </div>
         </c:if>
        </div>
          ...
          <div class="box box-primary">
            ...
            <div class="box-body">
                ...
                <div class="form-group${((not empty errors) and (not empty errors.name)) ? ' has-warning':''}">
                  <label>Tiêu đề</label>
                  <input type="text" name="category_name" value="${category.name}" class="input-lg form-control${((not empty errors) and (not empty errors.name)) ? ' form-control-warning':''}" placeholder="Nhập tiêu đề" required>
                ...
                <div class="form-group${((not empty errors) and (not empty errors.slug)) ? ' has-warning':''}">
                  ...
                  <input type="text" class="form-control${((not empty errors) and (not empty errors.slug)) ? ' form-control-warning':''}" name="category_slug" value="${category.slug}" placeholder="Nhập đường dẫn" >
                  <c:if test="${(not empty errors) and (not empty errors.slug)}">
                  <small class="form-text text-warning">${errors.slug}</small>
                  </c:if>
                </div>
                
                <div class="form-group${((not empty errors) and (not empty errors.position)) ? ' has-warning':''}">
                  <label>Thứ tự</label>
                  <input type="number" size="4" max="100" min="1" step="1" class="form-control${((not empty errors) and (not empty errors.slug)) ? ' form-control-warning':''}" name="category_position" value="${category.position}" placeholder="Nhập thứ tự" >
                  <c:if test="${(not empty errors) and (not empty errors.position)}">
                  <small class="form-text text-warning">${errors.position}</small>
                  </c:if>
                </div>
                ...
                <div class="form-group${((not empty errors) and (not empty errors.description)) ? ' has-warning':''}">
                  <label>Mô tả</label>
                  <textarea class="form-control${((not empty errors) and (not empty errors.description)) ? ' form-control-warning':''}" rows="3" name="category_description" placeholder="Nhập mô tả" required>${post.description}</textarea>
                ...
                <div id="form-category-image" class="form-group">
                  <label for="categoryImage">Icon</label>
                  <input id="categoryImage" name="category_image" class="form-control" value="${category.image}">
                  <button class="btn btn-primary" type="button" data-toggle="modal" data-target="#model-icon"><i class="fa ${category.image}"></i> Xem thêm...</button>
